...
public class FlowField
{
    ...
    public void CreateGrid()
    {
        ...
        for (int x = 0; x < grid.GetLength(0); x++)
        {
            ...
            {
                Vector3 worldPos = new Vector3(x * cellDiameter + cellRadius, y * cellDiameter + cellRadius);
        ...
        foreach(Cell curCell in grid)
        {
            ...
            foreach(var col in obstacle)
            {
                if (col.gameObject.layer.Equals(6))
                {
                    ...
                    continue;
                } else if (!hasIncreasedCost && col.gameObject.layer.Equals(7))
                {
    ...
    private List<Cell> GetNeighborCells(Vector2Int nodeIndex, List<GridDirection> directions)
    {
        ...
        foreach(Vector2Int curDir in directions)
        {
            Cell newNeighbor = GetCellAtRelativePos(nodeIndex, curDir);
            if(newNeighbor != null)
            {
